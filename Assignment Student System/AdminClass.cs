    ...
    internal class AdminClass
    {
       ...
       private static ArrayList no_repeat_list(ArrayList a)
        ...
        public static ArrayList sub_level_income(string level,string sub)
        {
            ArrayList subject = new ArrayList();
            con.Open(); 
            SqlCommand cmd = new SqlCommand("select totalFee_payment,month_payment,status_payment from payment where level_payment = '"+level+ "' and subject_payment = '"+sub+"'", con);
            SqlDataReader rd = cmd.ExecuteReader();
            while (rd.Read())
            {
                ...
                if (status == "approved")
                {
                    ...
                    lv_sub[0] = rd.GetInt32(0).ToString();
